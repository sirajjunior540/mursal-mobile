...
if ping -c 1 192.168.1.192 &> /dev/null; then
    echo "  ✅ Server 192.168.1.192 is reachable"
else
    echo "  ❌ Server 192.168.1.192 is NOT reachable"
fi

# Test 2: Port connectivity
echo "🧪 Testing port 8000 connectivity..."
if nc -z 192.168.1.192 8000 2>/dev/null; then
...
echo "   • Ensure the Django server is running on 192.168.1.192:8000"
echo "   • Check if tenant 'sirajjunior' is configured in Django"
echo "   • Verify CORS and ALLOWED_HOSTS settings in Django"
echo "   • Test from a browser: http://192.168.1.192:8000/api/v1/auth/"
...
echo "   ALLOWED_HOSTS = ['192.168.1.192', 'sirajjunior.192.168.1.192', '*']"
