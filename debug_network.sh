...
API_HOST="sirajjunior.172.20.10.2"
...
if ping -c 1 172.20.10.2 &> /dev/null; then
    echo "  ✅ Server 172.20.10.2 is reachable"
else
    echo "  ❌ Server 172.20.10.2 is NOT reachable"
fi

# Test 2: Port connectivity
echo "🧪 Testing port 8000 connectivity..."
if nc -z 172.20.10.2 8000 2>/dev/null; then
...
echo "   • Ensure the Django server is running on 172.20.10.2:8000"
echo "   • Check if tenant 'sirajjunior' is configured in Django"
echo "   • Verify CORS and ALLOWED_HOSTS settings in Django"
echo "   • Test from a browser: http://172.20.10.2:8000/api/v1/auth/"
...
echo "   ALLOWED_HOSTS = ['172.20.10.2', 'sirajjunior.172.20.10.2', '*']"
